...
class UsersPresentationUnitTest
{
    ...
    fun getUsers_isCorrect()
    {
        ...
        // given
        verify(view).drawUsers(users.map { UserViewModel(id = it.id, name = it.name) })
    }
    ...
    private fun createUsers(count: Int = 10): List<UserEntity> =
            (0..count).map { UserEntity(id = it.toLong(), name = "name$it", username = "username$it", email = "email@$it") }.toList()
}
